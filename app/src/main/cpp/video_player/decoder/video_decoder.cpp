...
std::list<MovieFrame*>* VideoDecoder::decodeFrames(float minDuration, int* decodeVideoErrorState) {
	...
	while (!finished) {
		ret = av_read_frame(pFormatCtx, &packet); // 读取一个压缩帧avpacket
		if (ret < 0) {
		...
		if (packet.stream_index == videoStreamIndex) { // 如果是视频帧
			this->decodeVideoFrame(packet, decodeVideoErrorState); // 解码一帧avpacket, 这里会根据mediacodec或者ffmpeg，走不同的解码
		} else if (packet.stream_index == audioStreamIndex) { // 如果是音频帧
